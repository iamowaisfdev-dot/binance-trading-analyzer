    ...
    if trade:
        ...
        try:
            if direction == "LONG":
                ...
                rr = round((entry - target) / (sl - entry), 2) if (sl - entry) != 0 else 0
        ...
        try:
            ...
            if result.get("trade"):
                ...
                if not risk_ok or not rr_ok:
                    print(" " * 60, end='\r')
                    print(DIM + f"  [{i}/{total}] {symbol} — signal rejected (RR:{round(rr,2)} / Risk:{result.get('risk_score')})" + RESET)
                    continue
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(DIM + "  Usage: python main.py <SYMBOL>")
        print(DIM + "  Examples:")
        print(DIM + "    python main.py ETH")
        print(DIM + "    python main.py SOLUSDT")
        print(DIM + "    python main.py BNB")
        print()
